...
RUN git fetch origin && git checkout -q "$CONTAINERD_VERSION"
...
ARG RUNC_VERSION
RUN git fetch origin && git checkout -q "$RUNC_VERSION"
...
ARG TINI_VERSION
RUN git fetch origin && git checkout -q "$TINI_VERSION"
...
RUN git clone https://github.com/rootless-containers/rootlesskit.git .
ARG ROOTLESSKIT_VERSION
RUN git fetch origin && git checkout -q "$ROOTLESSKIT_VERSION"
...
RUN git clone https://github.com/kolyshkin/go-swagger.git .
ARG GOSWAGGER_VERSION
RUN git fetch origin && git checkout -q "$GOSWAGGER_VERSION"
...
ARG CRIU_VERSION
RUN git fetch origin && git checkout -q "$CRIU_VERSION"
