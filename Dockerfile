...
RUN --mount=type=cache,sharing=locked,id=moby-systemd-aptlib,target=/var/lib/apt \
  ...
  apt-get update && apt-get install -y --no-install-recommends curl dbus dbus-user-session systemd systemd-sysv
RUN mkdir -p hack \
...
RUN --mount=type=bind,target=.,ro \
  ...
  CC=$(xx-info)-gcc CXX=$(xx-info)-g++ OUTPUT=/out ./hack/make/binary \
...
# > GO_LINKMODE=dynamic docker buildx bake binary
# or
# > make binary
# > make dynbinary
