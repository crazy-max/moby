...
func (i *ImageService) manifestIdentities(ctx context.Context, desc ocispec.Descriptor, multi *multiPlatformSummary) error {
	return i.manifestIdentitiesWithCachePolicy(ctx, desc, multi, true)
}

func (i *ImageService) manifestIdentitiesFromCache(ctx context.Context, desc ocispec.Descriptor, multi *multiPlatformSummary) error {
	return i.manifestIdentitiesWithCachePolicy(ctx, desc, multi, false)
}
...
func (i *ImageService) imageIdentityWithCachePolicy(ctx context.Context, desc ocispec.Descriptor, multi *multiPlatformSummary, computeOnCacheMiss bool) (*imagetypes.Identity, error) {
	baseIdentity, err := i.imageIdentityBase(ctx, desc)
	if err != nil {
		return nil, err
	}

	signature, err := i.manifestSignatureIdentityWithCachePolicy(ctx, desc, imageIdentityBestManifest(multi), computeOnCacheMiss)
	if err != nil {
		return nil, err
	}

	identity := cloneIdentity(baseIdentity)
	if signature != nil {
		if identity == nil {
...
func (i *ImageService) manifestIdentitiesWithCachePolicy(ctx context.Context, desc ocispec.Descriptor, multi *multiPlatformSummary, computeOnCacheMiss bool) error {
	if multi == nil {
		return nil
	}
	...
	for idx := range multi.Manifests {
		mfst := &multi.Manifests[idx]
		if mfst.Kind != imagetypes.ManifestKindImage || !mfst.Available || mfst.ImageData == nil {
			continue
		}

		im := multi.ManifestImages[mfst.Descriptor.Digest]
		signature, err := i.manifestSignatureIdentityWithCachePolicy(ctx, desc, imageManifestRef{Manifest: im, Platform: mfst.ImageData.Platform}, computeOnCacheMiss)
		if err != nil {
			return err
		}
...
func (i *ImageService) imageIdentityBase(ctx context.Context, desc ocispec.Descriptor) (*imagetypes.Identity, error) {
...
func imageIdentityBestManifest(multi *multiPlatformSummary) imageManifestRef {
	if multi == nil || multi.Best == nil {
		return imageManifestRef{}
	}
	return imageManifestRef{
		Manifest: multi.Best,
		Platform: multi.BestPlatform,
	}
}

func (i *ImageService) manifestSignatureIdentityWithCachePolicy(ctx context.Context, desc ocispec.Descriptor, ref imageManifestRef, computeOnCacheMiss bool) (*imagetypes.SignatureIdentity, error) {
	if ref.Manifest == nil {
		return nil, nil
	}

	cacheKey := imageIdentityCacheKey(desc.Digest.String(), ref.Manifest.Target().Digest.String(), platforms.FormatAll(ref.Platform))
	...
	if !ok && computeOnCacheMiss {
		v, err, _ := i.identity.flight.Do(cacheKey, func() (any, error) {
			...
			computedSignature, hasTransientVerificationError := i.computeManifestSignatureIdentity(ctx, desc, ref.Manifest, ref.Platform)
...
func (i *ImageService) computeManifestSignatureIdentity(ctx context.Context, desc ocispec.Descriptor, img *ImageManifest, platform ocispec.Platform) (*imagetypes.SignatureIdentity, bool) {
	if img == nil {
		return nil, false
	}

	signatureIdentity, err := i.signatureIdentity(ctx, desc, img, platform)
	if err != nil {
...
func cloneIdentity(identity *imagetypes.Identity) *imagetypes.Identity {
	if identity == nil {
		return nil
	}

	out := &imagetypes.Identity{
		Pull:  slices.Clone(identity.Pull),
		Build: slices.Clone(identity.Build),
	}
	if len(identity.Signature) > 0 {
		out.Signature = make([]imagetypes.SignatureIdentity, 0, len(identity.Signature))
		for _, sig := range identity.Signature {
			if cloned := cloneSignatureIdentity(&sig); cloned != nil {
				out.Signature = append(out.Signature, *cloned)
			}
		}
	}
	return out
...
func identityIsEmpty(identity *imagetypes.Identity) bool {
	return identity == nil || (len(identity.Build) == 0 && len(identity.Pull) == 0 && len(identity.Signature) == 0)
}
...
func (i *ImageService) warmImageIdentityCache(ctx context.Context, img c8dimages.Image) {
	...
	go func() {
		...
		if err := i.manifestIdentities(warmCtx, img.Target, multi); err != nil {
			log.G(warmCtx).WithError(err).WithField("image", img.Name).Debug("failed to build image identity cache in background")
		}
	}()
}
