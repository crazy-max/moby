...
import (
	...
	"github.com/docker/docker/pkg/linkmode"
	"github.com/sirupsen/logrus"
)

// ensureHomeIfStatic ensure $HOME to be set if linkmode.IsStatic is true.
...
func ensureHomeIfStatic() error {
	// Note: linkmode.IsStatic is only available for linux.
	// So we need to use them in this gcplogging_linux.go rather than in gcplogging.go
	if linkmode.IsStatic() && os.Getenv("HOME") == "" {
		home := homedir.Get()
